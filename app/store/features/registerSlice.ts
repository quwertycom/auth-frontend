...
const validationRules: ValidationRules = {
    ...
    if (formData.phone) {
      if (!phoneRegex.test(formData.phone)) {
        ...
        errors.push({
          input: 'phone',
          message:
            'Phone number must be at least 10 characters, remember to include country code',
        });
  ...
  3: (formData) => {
    if (!formData.termsAndConditions) {
      return [
        { input: 'termsAndConditions', message: 'You must agree to the terms' },
      ];
    }
    return [];
  },
...
const registerSlice = createSlice({
  ...
  reducers: {
    updateFormData: (
      state,
      action: PayloadAction<Partial<RegisterState['formData']>>,
    ) => {
...
export const { updateFormData, setErrors, clearErrors, resetRegistration } =
