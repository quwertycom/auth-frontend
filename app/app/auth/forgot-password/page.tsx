...
export default function ForgotPassword() {
  ...
  const transitionConfig = {
    ...
    animate: {
      ...
      transition:
        transition.normal.enterActive
          .transition,
    },
    exit: {
      ...transition.normal.leaveTo,
      transition:
        transition.normal.leaveActive
          .transition,
    },
  }
    ...
    <div className="relative h-screen w-screen">
      <div className={`relative flex h-screen w-screen flex-col items-center justify-center gap-8 ${status !== "idle" ? "scale-95 opacity-50 blur-sm" : "scale-100 opacity-100 blur-0"} transition-all duration-[350ms] ease-in-out-cubic`}>
        <Card className="w-full max-w-md">
          ...
          <CardFooter>
            <Button color="primary" variant="flat" className="w-full" onPress={handleSubmit}>
              Send reset link
            </Button>
          </CardFooter>
        </Card>
            ...
            >
              {status === 'loading' ? (
                ...
                <Card className='w-full max-w-xs'>
                  <CardHeader className='flex flex-col items-center gap-4 text-success pt-8'>
                    <MaterialSymbol symbol="check_circle" size={128} fill/>
                    <span className='text-3xl font-bold'>Success!</span>
                  </CardHeader>
                  <CardBody className='pb-8'>
                    <p className='text-center text-sm'>
                      We have sent a reset link to your email. Please check your inbox and spam folder. If you don&apos;t see it, please{' '}
                      <TransitionLink href="/app/auth/login" className="text-sm">
                        contact our support
                      </TransitionLink>
                      .
                  ...
                  <CardFooter>
                    <Button color="primary" variant="flat" className="w-full" onPress={() => navigate({href: '/app/auth/login'})}>
                      Back to login
                    </Button>
                  </CardFooter>
                </Card>
